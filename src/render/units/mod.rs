    ...
    fn get_figure_transforms(&self) -> Vec<Transform> {
        ...
        figures_translations
            ...
            .map(|figure_transaltion| {
                Transform::from_scale(scale_vec).with_translation(figure_transaltion)
            })
            .collect()
    }
}
...
pub fn selected(mut selected_query: Query<&mut Animation, (With<UnitFigure>, Added<Selected>)>) {
    for mut animation in selected_query.iter_mut() {
        println!("{:?}", animation);
        match animation.as_mut() {
            Animation::FigureAnimation {
                ref mut animation_type,
            } => {
                *animation_type = FigureAnimationType::Death;
            }
        }
    }
}

pub fn unselected(_commands: Commands, _unselected_query: RemovedComponents<Selected>) {}
